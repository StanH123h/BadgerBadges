...
// Achievement ID to icon mapping (using SVG data URLs)
 ...
 */
function generateEmojiSVG(emoji, title, tokenId = '', isTestNFT = false) {
  // Generate dynamic background color for test NFTs
  let gradientColors = {
    start: '#c5050c',
    end: '#9b0000'
  };

  if (isTestNFT && tokenId !== '') {
    // Generate HSL color based on token ID
    const normalizedId = parseInt(tokenId) - 10000; // 0-9999
    const hue = Math.floor((normalizedId / 10000) * 360); // 0-360
    gradientColors = {
      start: `hsl(${hue}, 70%, 65%)`,
      end: `hsl(${hue}, 70%, 45%)`
    };
...
/**
 * Read achievementId from contract for a given tokenId
 */
async function getAchievementIdFromContract(tokenId) {
  try {
    ...
    // We need to find the matching achievementId
...
export async function GET(request, { params }) {
  try {
    ...
    // Check if this is a test NFT (token ID 10000-19999)
    const isTestNFT = tokenIdNum >= 10000 && tokenIdNum < 20000;
    ...
    if (isTestNFT) {
      ...
      const metadata = {
        ...
        attributes: [
          ...
          {
            trait_type: 'Color Hue',
            value: Math.floor(((tokenIdNum - 10000) / 10000) * 360).toString(),
          },
        ],
      };
    ...
    const metadata = {
      ...
      image: generateEmojiSVG(achievement.icon, achievement.name, tokenId, false),
      external_url: `http://localhost:3000`,
      attributes: [
        {
          trait_type: 'Category',
